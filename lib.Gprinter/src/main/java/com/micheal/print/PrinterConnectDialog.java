...
public class PrinterConnectDialog extends Activity {
	...
	private PortParameters parameters=new PortParameters();
	...
	private void initPortParam() {
		...
		boolean[] state = intent.getBooleanArrayExtra(CONNECT_STATUS);
		for (int i = 0; i < 1; i++) {
	...
	void SetPortParamToView(PortParameters Param) {
		...
		map.put(ListViewAdapter.TITEL, Param.getUsbDeviceName());
...
//			map.put(ListViewAdapter.TITEL, getString(PrinterID[i]));
			map.put(ListViewAdapter.TITEL,mPortParam[i].getUsbDeviceName());
...
//            getBluetoothDevice();
	...
	private void okClicked(){
		parameters.setIpAddr("");
		parameters.setPortNumber(Integer.valueOf(""));
		parameters.setUsbDeviceName("");
		Intent intent = new Intent();
		Bundle bundle = new Bundle();
		bundle.putInt(GpPrintService.PORT_TYPE, parameters.getPortType());
		bundle.putString(GpPrintService.IP_ADDR, parameters.getIpAddr());
		bundle.putInt(GpPrintService.PORT_NUMBER, parameters.getPortNumber());
		bundle.putString(GpPrintService.BLUETOOT_ADDR, parameters.getBluetoothAddr());
		bundle.putString(GpPrintService.USB_DEVICE_NAME, parameters.getUsbDeviceName());
